...
- Boots a Spring Boot 4.0.7 app (Java 25, Gradle Groovy DSL) on port 8082 (`SERVER_PORT`).
- `GET /healthz` via `api/rest/HealthzController`.
- Postgres connection via `STOCKER_DB_URL`/`STOCKER_DB_USER`/`STOCKER_DB_PASSWORD` (defaults: localhost:5432/catalog, stocker/stocker).
...
- Package root: `com.stocker.catalog`.
